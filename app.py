...
st.set_page_config(page_title="Ultra NDT Optimizer v7", layout="wide")

# --- База данных ISO 17636-2 ---
...
materials = {"Алюминий": 1.0, "Сталь": 2.9, "Нерж. сталь": 3.0, "Титан": 1.7}

st.title("🔬 Ultra NDT & Microfocus Optimizer v7")

# --- ПАНЕЛЬ УПРАВЛЕНИЯ ---
with st.sidebar:
    st.header("💡 Источник")
    src_mode = st.radio("Режим фокуса", ["Микро (1-100 мкм)", "Макро (0.1-3 мм)"])
    if src_mode == "Микро (1-100 мкм)":
        f_mkm = st.slider("Фокус (мкм)", 1, 100, 20)
    else:
        f_mkm = st.slider("Фокус (мм)", 0.1, 3.0, 0.4, 0.1) * 1000
...
# --- МАТЕМАТИКА (АНАЛИТИЧЕСКИЙ ПОИСК МИНИМУМА) ---
# Оптимальное увеличение M_opt = ( (srb_det/f_mkm) + 1 ) ^ (1/3) -- упрощенно
# Точное решение Ug = Up в плоскости объекта:
# f * (M-1) = srb_det / M  => M^2 - M - (srb_det/f) = 0
# M_opt = (1 + sqrt(1 + 4 * (srb_det/f_mkm))) / 2
m_opt = (1 + np.sqrt(1 + 4 * (srb_det / f_mkm))) / 2
b_opt = fdd * (1 - 1/m_opt)

# Проверка границ
b_opt = np.clip(b_opt, 0.1, fdd * 0.9)
...
with c1:
    st.metric("Оптим. позиция", f"{b_opt:.1f} мм")
...
# --- ГРАФИК ---
b_axis = np.linspace(0.01, fdd * 0.95, 500)
